...
|---|---|---|
| `sourcing` | Aurias 2 market mapping, desk research, screening companies against the real criteria, flagging qualified leads (`.claude/agents/sourcing.md`) | Outreach drafting, valuation, editing verified data without certainty |
...
- **Formalize the sourcing workflow as an actual Claude Skill, not just an agent-brief markdown file** (research finding, 2026-09-16, from GTM practitioner content — Arise GTM/NativeGTM, Every.to's "compound engineering"). Right now `sourcing.md` is a well-written brief, but it's re-read and re-interpreted each time rather than packaged as a reusable Skill the way the most effective production setups do it (see the "50,000 leads" video precedent in the scraping research doc — a packaged Skill + verification sub-agents, not ad hoc prompting each time). Worth doing once the discovery-layer architecture (Outscraper etc.) is actually built, so the Skill covers the full pipeline, not just the enrichment half.
...
- **"Compound engineering" habit** (Dan Shipper/Every.to, 2026-09-16): when a one-off fix reveals a reusable pattern (e.g. the safe "trash + recreate" method for live Google Docs, the Sheets API batchUpdate pattern for multi-cell corrections), consider turning it into an actual reusable script/Skill, not just a memory note describing the lesson. Memory captures the "don't repeat this mistake" side well already; it doesn't yet capture the "here's the reusable tool" side.
