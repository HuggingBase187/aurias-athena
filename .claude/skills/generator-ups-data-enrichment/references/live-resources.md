...
- Tabs: **Market Map** (the working data — 460 companies as of 2026-09-18, confirmed by reading the full Company Name column via the Sheets API rather than `read_file_content`/`download_file_content`, which have both previously undercounted this sheet silently — most rows still only have Company Name + Website populated; was 461 until 2026-09-18, when a genuine exact duplicate, "Power Control Ltd," was found on the sheet and the less-complete of the two rows was removed per the never-delete-except-duplicates rule), **Qualified Leads** (destination for anything that clears screening via headcount or PBT — a copy, the source row stays on Market Map too, never removed), **Flagged to You** (added 2026-09-18 — destination for a "Needs more info" verdict, e.g. PBT qualifies but ownership is PE/large-group; same copy-not-move pattern as Qualified Leads, Daniel reviews and reclassifies himself), **Verification Log** (one row per enrichment batch, written by `generator-ups-data-verification`), **Legend & Sourcing Rules** (mirrors `references/data-template.md` — keep both in sync if either changes)
