...
**The rule, stated for any session, not just this one:** before deleting, inserting, or moving any row on Market Map — whether you're resolving a duplicate_scan.py candidate, doing a manual cleanup, or anything else structural — check both locks first:

1. `Automation Status!A2:C2` — must read `FREE`, not `IN_PROGRESS`.
2. `Batch Ledger` — no row may have Status `IN_PROGRESS`.

If either check fails, **do not delete anything.** Note the recommendation for later and come back once every batch shown as running has finished. This holds even if your edit feels unrelated to what the batches are working on — the whole point of the row-identity-verification rule in section 2 is that *any* structural change can shift row numbers out from under a concurrently-running batch, regardless of which rows either of you thinks you're touching.

**This is now a structural part of the tooling, not just a rule to remember:** `duplicate_scan.sh` prints both lock states to stderr before it prints its candidate list, specifically so this check can't be skipped by not thinking to run it separately. If you're building or using any other tool that can structurally edit this sheet, wire the same check in rather than relying on the person or agent using it to remember.

**Daniel's own standing rule for himself, same principle:** he doesn't make manual edits to the sheet while batches are running either — he waits until the batch finishes. The lock isn't just for tools and agents; it's the one shared signal everyone (human or agent, this session or another) checks before touching the sheet's structure.
